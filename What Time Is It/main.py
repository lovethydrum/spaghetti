...
button_list = [Button(button_locations[x-1][0], button_locations[x-1][1], pic_list[x-1]) for x in range(10)]
answer = ""
...
def refresh_question():
    ...
    random.shuffle(button_locations)
    random.shuffle(pic_list)
    button_list = [Button(button_locations[x - 1][0], button_locations[x - 1][1], pic_list[x - 1]) for x
                   in range(10)]
    game_window.blit(game_surface, (0, 0))
...
menu_button_1 = MenuButton(390, 230, GREEN)
ending_button_1 = MenuButton(240, 520, RED)
ending_button_2 = MenuButton(640, 520, GREEN)
ending_button_1.width, ending_button_1.height = 300, 150
ending_button_2.width, ending_button_2.height = 300, 150
...
def draw_game_over_menu():
    ...
    if score > high_score:
        high_score = score
    game_window.blit(game_surface, (0, 0))
    ending_button_1.draw_button()
    game_window.blit(cross, (335, 540))
    ending_button_2.draw_button()
...
while running:
    ...
    if game_state == "menu":
        ...
        refresh_question()
        pygame.event.clear()
        pygame.mixer.Sound.play(audio_dictionary[answer])
        print(answer)
        game_state = "play"
    ...
    #             running = False
        ...
        for event in ev:
            if event.type == pygame.QUIT:
                running = False
            if event.type == pygame.MOUSEBUTTONDOWN:
                if ending_button_1.rect.collidepoint(event.pos):
                    running = False
                if ending_button_2.rect.collidepoint(event.pos):
                    score = 0
                    counter = 60
                    game_state = "load"
